...
def balance_classes(*,
    ...
    foreground = foreground.groupby('class').apply(
        lambda d: d.sample(min_class_size,
    ...
    background = background.sample(n_background_points,
                                   random_state=random_state)
...
def load_images_and_create_data(*,
    ...
    frame_folders = [f for f in os.listdir(
        input_path) if os.path.isdir(os.path.join(input_path, f))]

    data = []
    for frame_folder in frame_folders:
        frame_path = os.path.join(input_path, frame_folder)
        segment_names = [name for name in os.listdir(
            frame_path) if name[1].isdigit()]
...
def evaluate(model, X, y):
    ...
    print('success rate: ',
          compute_identical_fraction(model.predict(X), y))
...
def load_model_and_generate_evaluation_images(*,
    ...
    for frame_folder in frame_folders:
        ...
        for camera_name in ['60', '180', '300']:
            ...
            cv.imwrite(filename=os.path.join(path, segments_filename),
...
if __name__ == "__main__":
    try:
        input_path = "./cube_image_set_p4_2020-09-16/"
        output_path = "./output_xbg_all_features/"
        ...
        load_images_and_create_data(input_path=input_path,
                                    output_filename='data.pkl')

        load_data_and_fit_model(input_filename='data.pkl',
                                output_filename="xgb_model.pkl",
                                feature_names=feature_names)

        load_model_and_generate_evaluation_images(model_filename="xgb_model.pkl",

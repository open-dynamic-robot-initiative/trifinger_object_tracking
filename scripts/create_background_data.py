    ...
    try:
        ...
        for frame_folder in frame_folders:
            ...
            for camera_name in ['60', '180', '300']:
                ...
                masks = {key: images[key] for key in images.keys()
                         if camera_name in key and key[1].isdigit() and key[0] != 'n'}
                
                for key in masks:
                    binary_mask = np.sum(masks[key], axis=2) == 0
                    eroded_binary_mask = ndimage.binary_erosion(binary_mask, structure=np.ones([10,10]))
                    image = eroded_binary_mask[..., None] * image
                
                cv.imwrite(os.path.join(frame_path, 'n' + camera_name + '.png'), image) 
